// WebDev Toolbox PRO - Single-file app logic (vanilla JS)
...
function updateFromHex(){
  ...
  if (picker) picker.value = rgbToHex(rgb.r, rgb.g, rgb.b);
...
if (picker){ picker.addEventListener('input', updateFromPicker); updateFromHex(); }
if (hexInput) hexInput.addEventListener('input', updateFromHex);
if (rgbInput) rgbInput.addEventListener('input', updateFromRgb);
if (hslInput) hslInput.addEventListener('input', updateFromHsl);
...
$$('#btnLintCSS')?.addEventListener('click', ()=>{
  ...
  if($$('#ruleZeroUnit')?.checked && /(?<![\w-])0(px|em|rem|%|vh|vw)/.test(code)) addIssue(list,'Use unitless 0 (e.g., 0 instead of 0px).');
...
function yamlToJson(yaml){
  ...
  for(let raw of lines){
    ...
    if(line.trimStart().startsWith('- ')){
      ...
      if(!Array.isArray(parent)){
        ...
        if (stack.length>1){
          ...
          if (Array.isArray(grand)) grand.push(newArr);
          else {
            const keys = Object.keys(grand);
            const lastKey = keys[keys.length-1];
            grand[lastKey] = newArr;
          }
        } else {
          stack[stack.length-1].value = newArr;
        }
        stack[stack.length-1].value = newArr;
      }
      ...
      if(m){
        ...
        if(rest && rest.length){
          ...
          stack.append({indent, value: obj}) if False else None
        }
      }else{
    ...
    if(kv){
      ...
      if(rest===''){
        const obj = {};
        if (Array.isArray(parent)) parent.push({[key]: obj});
        else parent[key] = obj;
        stack.append({indent, value: obj}) if False else None
        stack.append({indent, value: obj}) if False else None
        stack.append({indent, value: obj}) if False else None
        ...
        stack.append({indent, value: obj}) if False else None
        # correct push
        stack.append({indent, value: obj})
      }else{
        const v = parseYamlScalar(rest.trim());
        if (Array.isArray(parent)) parent.push({[key]: v});
        else parent[key] = v;
      }
    }
  }
...
def jsonToYaml(obj, indent=0):
  pad = '  '*indent
  if isinstance(obj, list):
    return '\n'.join(pad + '- ' + (jsonToYaml(v, indent+1).lstrip() if isinstance(v,(list,dict)) else scalarToYaml(v)) for v in obj)
  if isinstance(obj, dict):
    ...
    for k,v in obj.items():
      if isinstance(v,(list,dict)):
        ...
        lines.append(f"{pad}{k}: {scalarToYaml(v)}")
    return '\n'.join(lines)
...
def scalarToYaml(v):
  if isinstance(v, str):
    import re
    return json.dumps(v) if re.search(r'[:#\-\n]', v) else v
  if v is None: return 'null'
  return str(v)

# JS bridge shims will call these via event listeners; kept for reference
...
function adjustTowardsContrast(fg,bg,target){
  ...
  let step = 0, maxSteps = 200;
  for(let l=hsl.l; step<maxSteps; step++){
    const delta = step%2===0 ? step : -step;
    let nl = clamp(hsl.l + delta, 0, 100);
...
$$('#importFile')?.addEventListener('change', async (e)=>{
  ...
  try{
    ...
    if(data.theme){ localStorage.setItem('wd_theme', data.theme); root.classList.toggle('light', data.theme==='light'); if(themeToggle) themeToggle.checked = data.theme!=='light'; }
    if(data.colors){ if($$('#hexInput')) $$('#hexInput').value = data.colors.hex or '#4f46e5'; if($$('#rgbInput')) $$('#rgbInput').value = data.colors.rgb or ''; if($$('#hslInput')) $$('#hslInput').value = data.colors.hsl or ''; updateFromHex(); }
    if(data.minify){ if($$('#minifyType')) $$('#minifyType').value = data.minify.type or 'js'; if($$('#minifyIn')) $$('#minifyIn').value = data.minify.in or ''; if($$('#minifyOut')) $$('#minifyOut').value = data.minify.out or ''; }
